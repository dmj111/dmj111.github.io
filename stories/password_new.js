window.addEventListener('load', function () {
    function choose_from_list(num, lst) {
        var N = lst.length;
        return lst[Math.floor(N / (2**32) * num)];
    }
    function choose_word(num) {
        return choose_from_list(num, wordlist);
    }

    function choose_number(num) {
        return choose_from_list(num, "0123456789");
    }

    function choose_letter(num) {
        return choose_from_list(num, "abcdefghijklmnopqrstuvwxyz");
    }


    function choose_symbol(num) {
        return choose_from_list(num, "~`!@#$%^&*()-_+=[]\{}|;:'\",./<>?");
    }
    ...
    function make_passphrase() {
        var result = [];
        var n ;
        for(var i = 0; i <10; i += 1) {
            n = random_number();
            result.push(choose_word(n));
        }
        return result.join(" ");
    }

    function select_multiple(fcn, N, joiner) {
    ...
    function recompute() {
        document.getElementById('phrase').textContent = select_multiple(choose_word, 10, " ");
        document.getElementById('pin').textContent = select_multiple(choose_number, 10);
        document.getElementById('die100').textContent = Math.floor(random_number() / (2**32) * 100);
        document.getElementById('symbols').textContent = select_multiple(choose_symbol, 10, " ");
    }
    document.getElementById('redo').addEventListener('click', recompute);
